...
class Board
{
    ...
    private function initializeBlackPieces(): void
    {
        ...
        $this->fields['d8'] = new Piece(Side::WHITE, PieceType::QUEEN, new Position('d8'));
        $this->fields['e8'] = new Piece(Side::WHITE, PieceType::KING, new Position('e8'));
    }

    private function initializeWhitePieces()
    {
        $this->fields['a1'] = new Piece(Side::WHITE, PieceType::ROOK, new Position('a8'));
        $this->fields['h1'] = new Piece(Side::WHITE, PieceType::ROOK, new Position('h8'));
    ...
    public function removePieceAtPosition(Position $position): void
    {
        foreach ($this->pieces as $key => $piece) {
            if ($piece->position->equals($position)) {
                ...
                unset($this->fields[$position->toString()]);
            }
        }
    }
